...
//    val bcFeature = spark.sql("select * from " +
//      "(select b.* from " +
//      "(SELECT MAX(dt) as latest_dt FROM galaxy.wiwin_rec_user_feature_pv_follow_cart_search_catboost_v5 " +
//      "WHERE dt IS NOT NULL) a " +
//      "join " +
//      "galaxy.wiwin_rec_user_feature_pv_follow_cart_search_catboost_v5 b " +
//      "ON a.latest_dt=b.dt " +
//      "WHERE b.dt IS NOT NULL " +
//      ")mid " +
//      "where name_value not like '%cate2%' " +
//      "and name_value not like '%cate3%' " +
//      "and name_value not like '%brand%' "
//    )
//      .drop("dt")
//      .na.fill(0)
//
//    import spark.implicits._
//    val dataCatBoosted = bcFeature.rdd.map{
//      row =>
//
